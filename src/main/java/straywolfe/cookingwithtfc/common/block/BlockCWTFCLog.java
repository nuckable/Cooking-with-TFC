...
public class BlockCWTFCLog extends BlockLogNatural
{
	private static int damage;
	private static int logs;
	
	public BlockCWTFCLog()
	{
		super();
	...
	protected void processTree(World world, EntityPlayer entityplayer, int x, int y, int z, int meta, ItemStack is)
	{
		...
		for (int i = -2; i < 3; i++)
		{
			for (int j = -2; j < 3; j++)
			{
				if (world.getBlock(x + i, y, z + j) instanceof BlockCWTFCLog)
				{
					...
					for (int yDir = -1; yDir <= 1; yDir++)
					{
						for (int zDir = -1; zDir <= 1; zDir++)
						{
							if (!(xDir == 0 && yDir == 0 && zDir == 0))
							{
								...
								Block b2 = null;
								if (match instanceof BlockCWTFCBranch)
								{
									b = CWTFC_Core.getSourcedBranchForBranch(this, xDir, yDir, zDir);
									b2 = CWTFC_Core.getSourcedTerminalBranchForBranch(this, xDir, yDir, zDir);
								}
								if (b == match || b2 == match)
								{
									if (b == match)
									{
										...
										{
											((BlockCWTFCBranch) b).harvestBlock(world, entityplayer, i + xDir, y + j + yDir,
													k + zDir, meta);
										}
									}
									else
									{
										if (entityplayer != null)
										{
											((BlockCWTFCBranch) b2).harvestBlock(world, entityplayer, i + xDir, y + j + yDir,
													k + zDir, meta);
										}
									}
								}
